...
class Get5DayDailyForecastsUseCaseTest {

    @MockK
    internal lateinit var mockRemoteRepository: WeatherRepositoryImpl

    private lateinit var cut: Get5DayDailyForecastsUseCase

    @Before
    fun setUp() {
        MockKAnnotations.init(this)

        cut = Get5DayDailyForecastsUseCase(mockRemoteRepository)
    }
    ...
    fun `return DomainForecast`() {
        ...
        coEvery {
            mockRemoteRepository.get5DayDailyForecasts(cityKey)
        } returns domain
